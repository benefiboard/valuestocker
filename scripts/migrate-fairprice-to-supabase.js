// scripts/migrate-fairprice-to-supabase.js
...
async function migrateFairPriceData() {
  try {
    console.log('Fair Price 데이터 마이그레이션 시작...');

    // JSON 파일 읽기
    const fairPriceData = JSON.parse(fs.readFileSync(fairPriceJsonPath, 'utf8'));

    // stock_fairprice 데이터 마이그레이션
    console.log(
    ...
    console.log('Fair Price 데이터 마이그레이션 완료!');
  } catch (error) {

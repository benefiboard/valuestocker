// scripts/migrate-checklist-to-supabase-naver.js
...
// JSON 파일 경로
const checklistJsonPath = path.join(
  process.cwd(),
  'src/lib/finance/stock_checklist_naver_2025.json'
);
...
async function migrateData() {
  try {
    console.log('네이버 체크리스트 데이터 마이그레이션 시작...');

    // JSON 파일 읽기
    const checklistData = JSON.parse(fs.readFileSync(checklistJsonPath, 'utf8'));

    // stock_naver_checklist 데이터 마이그레이션
    console.log(
      `stock_naver_checklist 테이블에 ${Object.keys(checklistData).length}개의 항목 삽입 중...`
    );
    ...
    const checklistItems = Object.values(checklistData).map((item) => ({
      ...
      equityturnover: item.equityTurnover,

      // 재무 안정성 지표
      debtratio: item.debtRatio,
      interestbearingdebtratio: item.interestBearingDebtRatio,
      equityratio: item.equityRatio,

      // 현금 흐름 지표
      fcfratio: item.fcfRatio,
      opcashflowtorevenueratio: item.opCashFlowToRevenueRatio,
    ...
    for (let i = 0; i < checklistItems.length; i += BATCH_SIZE) {
      ...
      const { error } = await supabase.from('stock_naver_checklist').upsert(batch, {
    ...
    console.log('네이버 체크리스트 데이터 마이그레이션 완료!');
  } catch (error) {

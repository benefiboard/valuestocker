...
import SimpleBlogEditor, { BlogMedia } from '../components/SimpleBlogEditor';
...
export default function BlogPostEditor() {
    ...
    async function loadPost() {
      ...
      try {
        ...
        if (data) {
          ...
          // 미디어 아이템이 있는 경우 (JSON 파싱)
          if (data.media_items) {
            try {
              const parsedMedia = JSON.parse(data.media_items);
              setMediaItems(Array.isArray(parsedMedia) ? parsedMedia : []);
            } catch (e) {
              console.error('미디어 아이템 파싱 오류:', e);
            }
          }
        }
      } catch (error) {
  ...
  // 콘텐츠 변경 핸들러
  const handleContentChange = (newContent: string, newMediaItems: BlogMedia[]) => {
  ...
  const savePost = async (publish: boolean = false) => {
    ...
    try {
      ...
      // 미디어 아이템을 JSON 문자열로 변환
      const mediaItemsJson = JSON.stringify(mediaItems);

      const postData = {
        title,
        content,
        media_items: mediaItemsJson, // 미디어 아이템 저장
        updated_at: now,

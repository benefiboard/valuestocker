// src/app/profit-calculator/profitCalculate.ts

import { supabase } from '../../lib/supabaseClient';
import { ProfitCalculationParams, ProfitResult, ROEData, StockRawData } from './types';
...
export const calculateExpectedPBR = (params: ProfitCalculationParams): number => {
  const { settingROE, discountRate, sustainableYears } = params;

  // 예상 적정 PBR = [(1+설정ROE)/(1+할인율)]^n
...
export const calculateProfit = async (
  ...
  try {
    ...
    console.error('수익가치 계산 오류:', error);
    return null;
  }
};
